...
class MerckMolecularActivityDataset(Dataset):
    ...
    def get(cls, workdir=DEFAULT_DATA_DIR):
        if not isdir(os.path.join(workdir, "TrainingSet")):
            cls.prepare(workdir)
        df_1 = pd.read_csv(
            os.path.join(workdir, "TrainingSet/ACT2_competition_training.csv")
        )
        df_2 = pd.read_csv(
            os.path.join(workdir, "TrainingSet/ACT4_competition_training.csv")
        )
    ...
    def prepare(cls, workdir):
        try:
            ...
            kaggle.api.competition_download_file(
                competition="MerckActivity",
                file_name=cls.filename,
                path=workdir,
                quiet=True,
            )
        except OSError as e:
            kaggle_exception_msg = (
            ...
            if e.args[0] != kaggle_exception_msg:
                raise
            raise Exception(
                "You need to login to Kaggle to be able to download the Merck Molecular "
...
class CommunitiesAndCrimeDataset(Dataset):
    ...
    def get(cls, workdir=DEFAULT_DATA_DIR):
        ...
        X = df[
            df.columns[5:-1]
        ]  # First 5 columns are not predictive according to the dataset description
        y = df[df.columns[-1]]
...
class ParkinsonMultipleSoundRecordingDataset(Dataset):
    ...
    filenames = ["train_data.txt", "test_data.txt"]
    ...
    def get(cls, workdir=DEFAULT_DATA_DIR):
        ...
        """
        df = pd.read_csv(
            os.path.join(extracted_path, cls.filenames[0]), sep=",", header=None
        )
...
class FacebookCommentDataset(FacebookMetricsDataset):
    ...
    def get(cls, workdir=DEFAULT_DATA_DIR):
        ...
        select_comments = partial(
            FacebookMetricsDataset.select_label_col, name="comment"
        )
...
class StudentPerformanceNoPrevGradesBaseDataset(StudentPerformanceBaseDataset):
    @classmethod
    def get(cls, subject, workdir=DEFAULT_DATA_DIR):
        train, test = super(StudentPerformanceNoPrevGradesBaseDataset, cls).get(
            subject, workdir
        )
...
class StudentMathPerformanceNoPrevGradesDataset(
    StudentPerformanceNoPrevGradesBaseDataset
):
    @classmethod
    def get(cls, workdir=DEFAULT_DATA_DIR):
...
class StudentPortuguesePerformanceNoPrevGradesDataset(
    StudentPerformanceNoPrevGradesBaseDataset
):
    @classmethod
    def get(cls, workdir=DEFAULT_DATA_DIR):
        return super(StudentPortuguesePerformanceNoPrevGradesDataset, cls).get(
            "por", workdir
        )
