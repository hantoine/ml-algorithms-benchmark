...
def get_results_table():
    ...
    pattern = re.compile(
        RESULTS_DIR + r"/([a-zA-Z0-9]+)/([a-zA-Z0-9]+)/evaluation.json"
    )
    result_table = []
    ...
    for result_file in result_files:
        ...
        results["hp"] = ",".join(
            [
                f"{k}={v}" if type(v) in [str, list, type(None)] else f"{k}={v:.2f}"
...
def split_results_by_task(results):
    ...
    clf_interpretability_results = results[
        results["task"] == "classifier_interpretability"
    ].copy()
...
def print_results(format="markdown"):
    ...
    draw_cd_diagram(
        f"{RESULTS_DIR}/cd-diagram-classification-small-ds.png", clf_result_small_ds
    )
    ...
    draw_cd_diagram(
        f"{RESULTS_DIR}/cd-diagram-classification-big-ds.png", clf_result_big_ds
    )
    ...
    draw_cd_diagram(
        f"{RESULTS_DIR}/cd-diagram-regression-big-ds.png", reg_result_big_ds
    )
...
def print_rankings(task, results, format="default", use_short_alg_names=True):
    ...
    table = pd.pivot_table(
        results, values="model_rank", index="model", columns=["dataset"]
    )
...
def print_raw_results(task, results, format="default", use_short_alg_names=True):
    ...
    table = pd.pivot_table(
        results, values="score", index="model", columns=["dataset", "metric"]
    )
...
def print_all_datasets_results(datasets, results, format="default"):
    ...
    for dataset_name in dataset_names:
        ...
        table = (
            ...
            .set_index("model")[["score", "train_time"]]
        ...
        if format == "latex":
            print(
                "\\begin{figure}\\caption{Results on " + dataset_name[:-7] + " dataset}"
            )
